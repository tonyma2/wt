...
**Commits:** Use conventional commits — imperative mood, lowercase, no period (`fix:`, `feat:`, `feat!:` for breaking). Include `BREAKING CHANGE: explanation` in the footer when the impact isn't obvious from the subject. The first commit's subject becomes the PR title; update it if later commits or amends change the scope.
